...
if __name__=='__main__':
	...
	if flag==0:  # This section runs the MCMC for the various areas of interest, and stores the results
		mos_coeff,mos_results = bloodmeal.vector_coeff(msq_file,MCMC.poly_poiss_log_lik,poly_deg)
		bm_amean,bm_bmean,bm_results = bloodmeal.get_bloodmeal_sample(bm_file,MCMC.loglikelihood)
		...
		with open('bloodmeal_coeff_poly_deg(%d).pkl' %poly_deg, 'wb') as output:
			pickle.dump([bm_amean,bm_bmean],output)
		...
		import pylab
		bm_amean,bm_bmean =pickle.load(open('bloodmeal_coeff_poly_deg(%d).pkl' %poly_deg,'rb'))
		...
		if beta_flag ==0:  # Is flag supposed to be ODE_flag?  
			...
			beta1 = scipy.optimize.minimize(Seasonal_ODE.findbeta,.5,args=(rhs_func,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,flag,ODE_flag),method="COBYLA",bounds=[(0,1)],options={"disp":True,"iprint":2,"rhobeg":.25})
		...
		pylab.show()
		Y = Seasonal_ODE.run_ode(beta1.x,rhs_func,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,ODE_flag)
		
		
		if ODE_flag==2:
			Seasonal_ODE.eval_log_results(Y,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,bc_file)
		else:
			Seasonal_ODE.eval_ode_results(Y,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,bc_file,ODE_flag)
		pylab.show()
		...
		for i in range(len(poly_deg_vals)):
			...
			#bm_DIC[i] = pickle.load(open(bm_coeff_poly_deg(%d)_DIC.pkl' %poly_deg_vals[i],'rb')
		...
		bm_amean,bm_bmean =pickle.load(open('bloodmeal_coeff_poly_deg(%d).pkl' %bm_poly_deg,'rb'))
		...
		#beta1 = scipy.optimize.leastsq(Seasonal_ODE.findbeta,.5,args=(rhs_func,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,flag,ODE_flag))
		ODE_flag = 1
		beta1 = scipy.optimize.minimize(Seasonal_ODE.findbeta,.5,args=(rhs_func,bm_amean,bm_bmean,bc_coeff_mat,mos_coeff,tstart,tend,1,ODE_flag),method="COBYLA",bounds=[(0,1)],options={"disp":True,"iprint":2,"rhobeg":.25})
